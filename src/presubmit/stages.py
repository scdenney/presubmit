...
def stage_00b_metadata_clean(raw, pdf_path, output_dir):
    ...
    result = call_gemini(prompt, pdf_path, model_type="mechanical", temperature=0.0, thinking_budget=200, system_instruction=load_instruction("bureaucrat.txt"), step="00b_metadata_clean")
...
def stage_00b_2_metadata_math(metadata, output_dir):
    ...
    result = call_gemini(prompt, None, model_type="mechanical", temperature=0.0, thinking_budget=100, step="00b_2_metadata_math")
...
def stage_00c_contributions(pdf_path, metadata, output_dir):
    ...
    result = call_gemini(prompt, pdf_path, model_type="forensic", temperature=0.0, thinking_budget=1000, system_instruction=load_instruction("bureaucrat.txt"), step="00c_contributions")
...
def stage_01a_breaker(pdf_path, metadata, output_dir, contribs, appendix_text=None):
    ...
    prompt = inject_page_numbers(prompt, metadata)
    result = call_gemini(prompt, pdf_path, model_type="adversarial", temperature=0.2, thinking_budget=-1, system_instruction=load_instruction("researcher.txt"), step="01a_breaker")
...
def stage_01b_butcher(pdf_path, metadata, output_dir, contribs, appendix_text=None):
    ...
    if appendix_text:
        prompt += APPENDIX_INJECTION.replace("{{APPENDIX_CONTENT}}", appendix_text)
    prompt = inject_page_numbers(prompt, metadata)
    result = call_gemini(prompt, pdf_path, model_type="adversarial", temperature=0.2, thinking_budget=-1, system_instruction=load_instruction("researcher.txt"), step="01b_butcher")
...
def stage_01c_shredder(pdf_path, metadata, output_dir, contribs, appendix_text=None):
    ...
    if appendix_text:
        prompt += APPENDIX_INJECTION.replace("{{APPENDIX_CONTENT}}", appendix_text)
    prompt = inject_page_numbers(prompt, metadata)
    result = call_gemini(prompt, pdf_path, model_type="forensic", temperature=0.1, thinking_budget=-1, system_instruction=load_instruction("researcher.txt"), step="01c_shredder")
...
def stage_01d_collector(pdf_path, b1, s1, metadata, output_dir, appendix_text=None):
    ...
    if appendix_text:
        prompt += APPENDIX_INJECTION.replace("{{APPENDIX_CONTENT}}", appendix_text)
    prompt = inject_page_numbers(prompt, metadata)
    result = call_gemini(prompt, pdf_path, model_type="forensic", temperature=0.0, thinking_budget=-1, system_instruction=load_instruction("researcher.txt"), step="01d_collector")
...
def stage_01e_math_extract(pdf_path, contribs, metadata, output_dir, appendix_text=None):
    ...
    if appendix_text:
        prompt += APPENDIX_INJECTION.replace("{{APPENDIX_CONTENT}}", appendix_text)
    prompt = inject_page_numbers(prompt, metadata)
    result = call_gemini(prompt, pdf_path, model_type="forensic", temperature=0.0, thinking_budget=-1, system_instruction=load_instruction("bureaucrat.txt"), step="01e_math")
...
def stage_01e2_equation_extraction(pdf_path, metadata, output_dir):
    ...
    equations = call_gemini(
        extract_prompt, None,
        model_type="mechanical", temperature=0.0, thinking_budget=-1,
...
def stage_01fa_math_check(pdf_path, contribs, metadata, output_dir, appendix_text=None, equations=None):
    ...
    result = call_gemini(prompt, pdf_path, model_type="adversarial", temperature=0.1, thinking_level="high", system_instruction=load_instruction("researcher.txt"), step="01fa_math_check")
...
def stage_01fb_math_proofread(pdf_path, contribs, metadata, output_dir, appendix_text=None, equations=None):
    ...
    prompt = inject_page_numbers(prompt, metadata)
    prompt = _inject_equations(prompt, equations)
    result = call_gemini(prompt, pdf_path, model_type="adversarial", temperature=0.1, thinking_level="high", system_instruction=load_instruction("researcher.txt"), step="01fb_math_proofread")
...
def stage_01fc_math_audit(pdf_path, proofreader_output, contribs, metadata, output_dir, appendix_text=None, equations=None):
    ...
    prompt = inject_page_numbers(prompt, metadata)
    prompt = _inject_equations(prompt, equations)
    result = call_gemini(prompt, pdf_path, model_type="adversarial", temperature=0.1, thinking_level="high", system_instruction=load_instruction("researcher.txt"), step="01fc_math_audit")
...
def stage_01fd_math_sober(pdf_path, rederiver_output, proofreader_output, auditor_output, contribs, metadata, output_dir, appendix_text=None, equations=None, mathpix_raw=None):
    ...
    result = call_gemini(prompt, None if mathpix_raw else pdf_path, model_type="synthesis", temperature=0.1, thinking_level="high", system_instruction=load_instruction("researcher.txt"), step="01fd_math_sober")
...
def stage_01g_the_void(pdf_path, contribs, metadata, output_dir, appendix_text=None):
    ...
    if appendix_text:
        prompt += APPENDIX_INJECTION.replace("{{APPENDIX_CONTENT}}", appendix_text)
    prompt = inject_page_numbers(prompt, metadata)
    result = call_gemini(prompt, pdf_path, model_type="adversarial", temperature=0.3, thinking_budget=-1, system_instruction=load_instruction("researcher.txt"), step="01g_the_void")
...
def stage_01_code_gonzo(combined_pdf, metadata, output_dir):
    ...
    result = call_gemini(prompt, combined_pdf, model_type="adversarial", temperature=0.5,
...
def stage_01_code_compiler(g1, g2, g3, metadata, output_dir):
    ...
    result = call_gemini(prompt, None, model_type="mechanical", temperature=0.0, thinking_budget=-1,
...
def stage_01_code_checker(consolidated, metadata, output_dir, code_pdf):
    ...
    prompt = inject_page_numbers(prompt, metadata, is_code_stage=True)
    result = call_gemini(prompt, code_pdf, model_type="forensic", temperature=0.1, thinking_budget=1000,
...
def stage_01_code_list(consolidated, checker_output, metadata, output_dir):
    ...
    prompt = inject_page_numbers(prompt, metadata, is_code_stage=True)
    result = call_gemini(prompt, None, model_type="forensic", temperature=0.5, thinking_level="low",
...
def stage_04b_data_editor(findings, metadata, output_dir, paper_review_context=None):
    ...
    prompt = inject_page_numbers(prompt, metadata, is_code_stage=True)
    result = call_gemini(prompt, None, model_type="adversarial", temperature=0.5, thinking_level="low", system_instruction=load_instruction("thinker.txt"), step="04b_data_editor")
...
def stage_01o_summarizer(br1, bu1, sh1, col, alg, math, void, contribs, metadata, output_dir, math_analysis=None):
    ...
    result = call_gemini(prompt, None, model_type="forensic", temperature=0.3, thinking_level="medium", system_instruction=load_instruction("bureaucrat.txt"), step="01o_summarizer")
...
def stage_02a_numbers(pdf_path, issues, metadata, output_dir, equations=None):
    ...
    prompt = inject_page_numbers(prompt, metadata)
    prompt = _inject_equations(prompt, equations)
    result = call_gemini(prompt, pdf_path, model_type="forensic", temperature=0.3, thinking_level="medium", system_instruction=load_instruction("bureaucrat.txt"), step="02a_numbers")
...
def stage_02b_compiler_1(issues, numbers, output_dir):
    ...
    result = call_gemini(prompt, None, model_type="mechanical", temperature=0.0, thinking_budget=-1, system_instruction=load_instruction("bureaucrat.txt"), step="02b_compiler_1")
...
def stage_02c_blue_team(pdf_path, compiled_issues_v1, contribs, metadata, output_dir):
    ...
    prompt = inject_page_numbers(prompt, metadata)
    result = call_gemini(prompt, pdf_path, model_type="synthesis", temperature=0.2, thinking_level="medium", system_instruction=load_instruction("thinker.txt"), step="02c_blue_team")
...
def stage_02d_compiler_2(compiled_issues_v1, blue_team, output_dir):
    ...
    result = call_gemini(prompt, None, model_type="mechanical", temperature=0.0, thinking_budget=-1, system_instruction=load_instruction("bureaucrat.txt"), step="02d_compiler_2")
...
def stage_02e_assessment(pdf_path, compiled_issues_v2, metadata, output_dir):
    ...
    prompt = inject_page_numbers(prompt, metadata)
    result = call_gemini(prompt, pdf_path, model_type="synthesis", temperature=0.4, thinking_level="high", system_instruction=load_instruction("thinker.txt"), step="02e_assessment")
...
def stage_02f_compiler_3(compiled_issues_v2, assessment, output_dir):
    ...
    result = call_gemini(prompt, None, model_type="mechanical", temperature=0.0, thinking_budget=-1, system_instruction=load_instruction("bureaucrat.txt"), step="02f_compiler_3")
...
def stage_02g_list_v1(pdf_path, compiled_issues_v3, metadata, output_dir):
    ...
    prompt = inject_page_numbers(prompt, metadata)
    result = call_gemini(prompt, pdf_path, model_type="synthesis", temperature=0.5, thinking_level="high", system_instruction=load_instruction("thinker.txt"), step="02g_list_v1")
...
def stage_03a_checker_1(pdf_path, list_v1, metadata, output_dir, equations=None):
    ...
    prompt = inject_page_numbers(prompt, metadata)
    prompt = _inject_equations(prompt, equations)
    result = call_gemini(prompt, pdf_path, model_type="forensic", temperature=0.5, thinking_level="low", system_instruction=load_instruction("thinker.txt"), step="03a_checker_1")
...
def stage_03b_external(pdf_path, list_v1, metadata, output_dir):
    ...
    prompt = inject_page_numbers(prompt, metadata)
    result = call_gemini(prompt, pdf_path, model_type="mechanical", temperature=0.0, thinking_budget=-1, system_instruction=load_instruction("bureaucrat.txt"), step="03b_external")
...
def stage_03c_list_v2(list_v1, fact_check, ext_check, metadata, output_dir):
    ...
    prompt = prompt.replace("{{CITATION}}", get_citation_block(metadata))
    result = call_gemini(prompt, None, model_type="forensic", temperature=0.3, thinking_level="low", system_instruction=load_instruction("bureaucrat.txt"), step="03c_list_v2")
...
You can also reference this Appendix, **but you must not confuse it with any appendices in the original text.**
    ...
    prompt = inject_page_numbers(prompt, metadata)
    result = call_gemini(prompt, pdf_path, model_type="synthesis", temperature=0.4, thinking_level="high", system_instruction=load_instruction("thinker.txt"), step="04a_reviewer")
...
def stage_05a_checker_2(pdf_path, review, metadata, output_dir, equations=None):
    ...
    prompt = inject_page_numbers(prompt, metadata)
    prompt = _inject_equations(prompt, equations)
    result = call_gemini(prompt, pdf_path, model_type="forensic", temperature=0.5, thinking_level="low", system_instruction=load_instruction("thinker.txt"), step="05a_checker_2")
...
def stage_05b_checker_3(pdf_path, review, metadata, output_dir):
    ...
    prompt = inject_page_numbers(prompt, metadata)
    result = call_gemini(prompt, pdf_path, model_type="forensic", temperature=0.5, thinking_level="low", system_instruction=load_instruction("thinker.txt"), step="05b_checker_3")
...
def stage_05c_reviser(review, chk2, chk3, metadata, output_dir):
    ...
    prompt = prompt.replace("{{CITATION}}", get_citation_block(metadata))
    result = call_gemini(prompt, None, model_type="forensic", temperature=0.3, thinking_level="low", system_instruction=load_instruction("thinker.txt"), step="05c_reviser")
...
def stage_06_legal(review, metadata, output_dir):
    ...
    result = call_gemini(prompt, None, model_type="forensic", temperature=0.0, thinking_budget=-1, system_instruction=load_instruction("bureaucrat.txt"), step="06_legal")
...
def stage_07_formatter(review, legal, metadata, output_dir, appendices=None):
    ...
    result = call_gemini(prompt, None, model_type="mechanical", temperature=0.3, thinking_level="low", system_instruction=load_instruction("bureaucrat.txt"), step="07_formatter")
...
def stage_08a_alchemist(pdf_path, formatted, metadata, output_dir, writer_mode=True):
    ...
    prompt = prompt.replace("{{CITATION}}", get_citation_block(metadata))
    prompt = inject_page_numbers(prompt, metadata)
    result = call_gemini(prompt, pdf_path, model_type="synthesis", temperature=0.5, thinking_level="high", system_instruction=load_instruction("thinker.txt"), step="08a_alchemist")
...
def stage_08b_polisher(alchemist_text, metadata, output_dir):
    ...
    result = call_gemini(prompt, None, model_type="forensic", temperature=0.3, thinking_level="low", system_instruction=load_instruction("bureaucrat.txt"), step="08b_polisher")
...
def stage_09a_proofreader(pdf_path, metadata, output_dir, writer_mode=True):
    ...
    prompt = inject_page_numbers(prompt, metadata)
    result = call_gemini(prompt, pdf_path, model_type="forensic", temperature=0.1, thinking_level="low", system_instruction=load_instruction("bureaucrat.txt"), step="09a_proofreader")
...
def stage_09b_proofread_clean(raw, metadata, output_dir, writer_mode=True):
    ...
    result = call_gemini(prompt, None, model_type="mechanical", temperature=0.3, thinking_level="low", system_instruction=load_instruction("bureaucrat.txt"), step="09b_proofread_clean")
...
def stage_09c_copyedit(pdf_path, review, polished_note, inst, metadata, output_dir, writer_mode=True):
    ...
    prompt = inject_page_numbers(prompt, metadata)
    result = call_gemini(prompt, pdf_path, model_type="synthesis", temperature=0.2, thinking_level="high", system_instruction=load_instruction("thinker.txt"), step="09c_copyedit")
